...
fn resize_state(cached: &RawState, new_seq_budget: usize) -> Result<RawState> {
...
impl Model {
    ...
    fn start_generation_(
        ...
        let base = resize_state(voice, seq_budget)?;
    ...
    fn next_chunk_(&mut self) -> Result<Option<usize>> {
        ...
        let prev_latent = Tensor::from_vec(vec![f32::NAN; ldim], (1, 1, ldim), &CPU)?;
    ...
    fn generation_step_(&mut self) -> Result<Option<js_sys::Float32Array>> {
        ...
        let (next_latent, audio_chunk, is_eos) =
            dispatch!(&self.inner, &mut state.tts_state, |m, s| {
                let (next_latent, is_eos) = m.generate_step(s, &state.prev_latent, rng)?;
...
impl Model {
    ...
    pub fn new(
        ...
        Self::new_(model_weights, tokenizer_json, config_json, quant, lang).map_err(js_err)
    }

...
fn resolve(config: Option<&str>, temperature: f32) -> xn::Result<Artifacts> {
    ...
    match config {
        ...
        Some(repo_id) => {
            ...
            Ok(Artifacts {
                cfg,
                model_path: repo.get("model.q8.gguf").map_err(xn::Error::msg)?,
                tokenizer_path: repo.get("tokenizer.model").map_err(xn::Error::msg)?,
                voices: vec![],
            })
        }
        ...
        None => {
            ...
            let model_path = repo
                ...
                .map_err(|e| xn::Error::msg(e).with_path(DEFAULT_MODEL_FILE))?;
            let tokenizer_path = repo.get("tokenizer.model").map_err(xn::Error::msg)?;
            let mut voices = vec![];
            for &voice in POCKET_TTS_VOICES {
                if let Ok(path) = repo.get(&format!("embeddings/{voice}.safetensors")) {
...
fn hub(repo_id: &str) -> xn::Result<hf_hub::api::sync::ApiRepo> {

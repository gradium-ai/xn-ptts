...
impl<Q: BackendQ> SynthOf<Q> {
    ...
    /// computes internally and what the `create_voice` example writes to a
    /// file. Callers that compute or cache embeddings themselves (`ptts-pyo3`
    /// hands one over from numpy) use this.
    ///
